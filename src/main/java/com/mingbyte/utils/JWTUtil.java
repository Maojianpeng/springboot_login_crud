...
public enum JWTUtil {
  ...
  public String generate(Map<String, Object> map, String sign, Date exp) {
    ...
    JwtBuilder jwtBuilder = Jwts.builder().setClaims(map).signWith(SignatureAlgorithm.HS512, sign);
  ...
  public JWT check(String token, String sign) throws ExpiredJwtException, SignatureException {
    ...
    try {
      // parse the token.
      Map<String, Object> body = Jwts.parser().setSigningKey(sign).parseClaimsJws(token).getBody();
  ...
  public class JWT {
    ...
    /** 结果解析状态 0：正常 1：TOKEN 已过期 2：签名验证失败 3：未知其它异常 */
    private int status;
    ...
    public String toString() {
      return "JWT{" + "status=" + status + ", body=" + body + '}';
    }
  }
}
